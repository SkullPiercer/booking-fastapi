...
async def create_user(user: UserCreateSchema, db: DBDep):
    try:
...
async def create_user_token(
    user: UserTokenSchema, response: Response, db: DBDep
):
...
async def get_data_auth_only(user_id: UserIdDep, db: DBDep):
...
async def set_avatar(user_id: UserIdDep, db: DBDep, file: DownloadFileDep):
