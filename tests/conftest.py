...
async def create_hotels(async_client):
    ...
    await async_client.post(url="/hotels/bulk", json=hotels)


@pytest.fixture(scope="session", autouse=True)
async def create_rooms(async_client, create_facilities, create_hotels):
    rooms = await read_file(ROOMS_FIXTURE_PATH)
    await async_client.post(
        url=f"hotels/{randint(1, 20)}/rooms/bulk", json=rooms
    )
...
async def create_facilities(async_client):
    ...
    await async_client.post(url="/facilities/bulk", json=facilities)
...
async def hotel(db):
    hotel = await db.hotels.create(
        HotelCreateSchema(
            title="Ну крутой отель", location="И крутое описание"
        )
    )
    await db.commit()
    return hotel
...
async def user_client(async_client, root_user):
    await async_client.post(url="/users/login", json=ROOT_USER_DATA)
